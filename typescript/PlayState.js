...
var PlayState = (function (_super) {
    ...
    PlayState.prototype.create = function () {
        ...
        if (PlayState.roomsCleared <= 7) {
            for (var i = 0; i < chestX.length; i++) {
                if (Math.random() < chestProb[i]) {
                    var chest = this.add.sprite(chestX[i] * 32, chestY[i] * 32, "chest", 0, this.chestGroup);
        ...
        var hudBG = this.game.add.bitmapData(96, 480);
    ...
    PlayState.prototype.update = function () {
        ...
        for (var i = 0; i < this.weaponHud.length; i++) {
            if (Player.weaponAmmo[i] <= 0) {
                this.weaponHud[i].visible = false;
            }
            else {
                this.weaponHud[i].visible = true;
            }
        }
    ...
    PlayState.prototype.ghostVsRock = function (ghost, rock) {
        if (ghost.x > 360) {
            ghost.x -= 1;
        }
        else {
    ...
    PlayState.prototype.bulletVsRock = function (bullet, rock) {
        if (bullet.health == 2) {
            bullet.kill();
        }
    };
    PlayState.prototype.bulletVsChest = function (bullet, chest) {
    ...
    PlayState.prototype.bulletVsGhost = function (bullet, ghost) {
    ...
    PlayState.prototype.playerVsBullet = function (player, bullet) {
        PlayState.player.onCollision();
        bullet.kill();
    };
    PlayState.prototype.playerVsItem = function (player, item) {
    ...
    PlayState.prototype.playerVsGoal = function (player, goal) {
    ...
    PlayState.prototype.processHandler = function () {
        return true;
    };
    return PlayState;
}(Phaser.State));
